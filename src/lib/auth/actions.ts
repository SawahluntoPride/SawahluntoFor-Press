...
export async function login(prevState: unknown, formData: FormData) {
  ...
  const rl = checkRateLimit(`login:${parsed.data.email}`);
  if (!rl.allowed) {
    return {
      errors: {},
      message: `Terlalu banyak percobaan. Coba lagi dalam ${rl.retryAfterSeconds} detik.`,
    };
  }

  // ...lanjut kode yang sudah ada (cari user, cek password, dst)
