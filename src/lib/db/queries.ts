...
/**
 * Cari pengajuan berdasarkan nomor referensi untuk halaman /status.
 * Publik (tanpa login) — hanya kembalikan info yang aman ditampilkan,
 * bukan seluruh isi proposal.
 */
export async function getProposalStatusByReference(referenceNumber: string) {
  const proposal = await prisma.proposal.findFirst({
    where: { referenceNumber: referenceNumber.trim() },
    include: {
      org: { select: { name: true } },
      documents: { select: { name: true, verified: true } },
    },
  });
  if (!proposal) return null;
  ...
  return {
    ...
    orgName: proposal.org?.name ?? null,
